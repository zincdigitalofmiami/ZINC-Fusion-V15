...
def load_historical_states(
    conn, horizon: int, current_date: datetime, min_lookback: int = MIN_LOOKBACK_DAYS
) -> List[MarketState]:
...
def load_regime_history(conn, start_date: datetime, end_date: datetime) -> Dict:
    ...
    return {
        row[0].date() if hasattr(row[0], "date") else row[0]: row[1] for row in rows
    }
...
def calculate_driver_correlation(
    current_ranks: Dict[str, int], historical_ranks: Dict[str, int]
) -> float:
...
def calculate_similarity(
    current: MarketState, historical: MarketState
    ...
    # Weighted sum
    overall = sum(
        components[key] * SIMILARITY_WEIGHTS[key] for key in SIMILARITY_WEIGHTS
    )
...
def save_analogs(
    conn, current_date: datetime, horizon: int, analogs: List[AnalogResult]
) -> int:
    ...
    for analog in analogs:
        batch.append(
            (
                ...
                float(analog.actual_outcome)
                if analog.actual_outcome is not None
                else None,
...
def run_analog_search(
    horizon: int, top_n: int = 5, dry_run: bool = False
) -> List[AnalogResult]:
    ...
    try:
        ...
        for i, analog in enumerate(analogs, 1):
            outcome_str = (
                f"{analog.actual_outcome:+.1%}" if analog.actual_outcome else "N/A"
            )
            ...
            logger.info(
                f"     Components: p50={analog.components['p50_sim']:.2f}, "
...
def main():
    ...
    parser.add_argument(
        "--top-n", type=int, default=5, help="Number of analogs to return (default: 5)"
    )
