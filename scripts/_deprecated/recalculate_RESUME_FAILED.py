#!/usr/bin/env python3
"""
RECALCULATE ONLY THE FAILED SYMBOLS - WITH FRESH CONNECTIONS

The main script had a connection timeout. This processes only the failed symbols
with a fresh connection per symbol to prevent cascade failures.
"""
...
# FAILED SYMBOLS from the original run (42-105 + GVZ/HE edge cases)
FAILED_SYMBOLS = [
    "GVZ", "HE",  # Edge cases near the timeout
...
def process_symbol(symbol: str) -> int:
    """Process ONE symbol with its own connection."""
    conn = None
    try:
        conn = get_connection()
...
def main():
    """Process only the failed symbols, each with its own connection."""
    print("\n" + "=" * 70)
    print("RESUMING FAILED SYMBOLS - FRESH CONNECTION PER SYMBOL")
    print("=" * 70)
    print(f"\nProcessing {len(FAILED_SYMBOLS)} failed symbols...\n")
    ...
    for i, symbol in enumerate(FAILED_SYMBOLS):
        print(f"[{i+1}/{len(FAILED_SYMBOLS)}] {symbol}...", end=" ", flush=True)
        try:
            updated = process_symbol(symbol)
    ...
    print("=" * 70)
    print(f"Successful: {successful}/{len(FAILED_SYMBOLS)}")
