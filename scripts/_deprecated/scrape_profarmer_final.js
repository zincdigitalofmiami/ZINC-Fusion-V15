...
async function scrapeAll() {
  ...
  // Type with keyboard (THIS IS CRITICAL)
  ...
  for (const section of sections) {
    ...
    for (let pageNum = 1; pageNum <= 500; pageNum++) {
      ...
      try {
        ...
        console.log(`  Page ${pageNum}: ${newArticles.length} articles`);
        ...
        for (const article of newArticles) {
          ...
          try {
            ...
            const pageData = await page.evaluate(() => {
              ...
              // Author
              let author = '';
              const authorEl = document.querySelector('.Page-authorName a, .byline a, [rel="author"]');
              if (authorEl) author = authorEl.textContent?.trim() || '';
              
              // Content - use the correct selector!
              let content = '';
              const contentEl = document.querySelector('.Page-articleBody, .RichTextArticleBody, .RichTextBody');
              if (contentEl) content = contentEl.textContent?.trim()?.slice(0, 50000) || '';
              
              if (!content || content.length < 100) {
                const pageContent = document.querySelector('.Page-content');
                if (pageContent) content = pageContent.textContent?.trim()?.slice(0, 50000) || '';
              }
          ...
          try {
            ...
            if (!e.message.includes('duplicate')) console.log(`    ! ${e.message.slice(0, 50)}`);
            else totalSkipped++;
          }
        }
