...
print("\n1. FEATURES.OPTIONS_1D:")
cur.execute(
    """
    SELECT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'features' AND table_name = 'options_1d'
    )
"""
)
...
if exists:
    cur.execute("SELECT COUNT(*) FROM features.options_1d")
    print(f"   Row count: {cur.fetchone()[0]:,}")

# Check mkt.options_1d (source data)
print("\n2. MKT.OPTIONS_1D (source for Phase 1):")
cur.execute(
    """
    SELECT EXISTS (
        SELECT 1 FROM information_schema.tables
...
HARD MISSING (blocking):
  - features.options_1d: Table does not exist → Run Phase 1
  - mkt.options_1d: Check if source data exists for IV/Greeks
