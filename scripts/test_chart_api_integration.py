...
def test_endpoint(
    ...
    try:
        ...
        if "bars" in data and len(data["bars"]) > 1:
            ...
            timestamps = [d.get("timestamp") or d.get("event_date") for d in data["data"]]
            ordering_passed = timestamps == sorted(timestamps)
        ...
        return {
            ...
            "message": f"Response OK" + (
                "" if passed else f" (issues: {', '.join(['missing_fields' if not fields_passed else '', 'ordering' if not ordering_passed else '', 'performance' if not perf_passed else ''])})"
            ),
            "status_code": response.status_code,
            ...
            "data_count": len(data.get("bars", data.get("data", data.get("series", []))))
        }
...
def check_data_completeness(endpoint: str, params: Dict) -> Dict:
    ...
    try:
        ...
        return {
            "passed": len(gaps) == 0,
            "message": f"Found {len(gaps)} potential gaps" if gaps else "No significant gaps",
            "gap_count": len(gaps),
...
def main():
    ...
    results["completeness_intraday"] = check_data_completeness("/api/zl/intraday", {"hours": 24})
    results["completeness_1h"] = check_data_completeness("/api/zl/price-1h", {"hours": 168})
    results["completeness_1d"] = check_data_completeness("/api/zl/price-1d", {"days": 90})
