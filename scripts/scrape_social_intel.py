...
# Truth Social profiles (posts endpoint not available - using Twitter instead)
TRUTH_SOCIAL_PROFILES = [
    {"handle": "realDonaldTrump", "name": "Donald Trump", "specialist": "trump_effect", "priority": 0},
]

# Instagram - DISABLED per user request (Twitter/X, LinkedIn, Facebook, Truth Social only)
...
def fetch_truthsocial_posts(handle: str, api_key: str, limit: int = 20) -> List[Dict]:
    ...
    Note: Truth Social posts endpoint not available - use Twitter as Trump posts same content.
    """
    # Truth Social posts endpoint not available in current API
    logger.debug(f"Truth Social skipped for {handle} - posts endpoint not available (using Twitter instead)")
    return []


# Instagram fetch function removed - platform not requested
...
# Instagram post processor removed - platform not requested
...
def scrape_tier(
    ...
    # Instagram disabled per user request - only Twitter/X, LinkedIn, Facebook, Truth Social
