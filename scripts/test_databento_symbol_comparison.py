...
def compare_symbols(bars_c: List[Bar], bars_n: List[Bar]) -> Dict:
    ...
    for ts in sorted(common_ts):
        ...
        if c_bar.volume > 0 or n_bar.volume > 0:
            vol_diff_pct = abs(c_bar.volume - n_bar.volume) / max(c_bar.volume, n_bar.volume, 1) * 100
            volume_diffs.append({
                "timestamp": c_bar.timestamp.isoformat(),
    ...
    return {
        ...
        "price_stats": {
            ...
            "p95_diff_pct": sorted(pct_diffs)[int(len(pct_diffs) * 0.95)] if pct_diffs else 0,
            "bars_lt_0_1_pct": sum(1 for d in pct_diffs if d < 0.1),
            "bars_lt_0_1_pct_pct": sum(1 for d in pct_diffs if d < 0.1) / len(pct_diffs) * 100 if pct_diffs else 0
        },
        "roll_candidates": roll_candidates[:10],  # Top 10
        "volume_stats": {
            "max_diff_pct": max([d["vol_diff_pct"] for d in volume_diffs]) if volume_diffs else 0,
            "avg_diff_pct": sum([d["vol_diff_pct"] for d in volume_diffs]) / len(volume_diffs) if volume_diffs else 0
        },
        "timing": {
