...
from fusion.db import execute_batch
...
def test_24h_boundary(engine) -> Dict:
    ...
    return {
        "passed": passed,
        "message": f"Found {count} rows in last 24h from historical jobs" + (" (should be 0)" if passed else " (violates 24h boundary)"),
...
def test_source_conflict(engine) -> Dict:
    ...
    return {
        "passed": passed,
        "message": f"Live data: {live_count} rows, Overlaps: {overlap_count}" + (" (no conflict)" if passed else " (potential conflict)"),
...
def test_backfill(engine) -> Dict:
    ...
    return {
        "passed": passed,
        "message": f"Coverage: {coverage_pct:.1f}% ({covered_days}/{expected_days} days)" + (" (meets threshold)" if passed else " (below 80% threshold)"),
