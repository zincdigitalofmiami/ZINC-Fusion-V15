...
def fetch_market_data(conn) -> pd.DataFrame:
    ...
    logger.info(f"Loaded {len(df)} days of ZL data ({df.index.min().date()} to {df.index.max().date()})")
    return df
...
def fetch_specialist_signals(conn) -> pd.DataFrame:
    ...
    logger.info(f"Loaded signals for {len(df['bucket'].unique())} specialists, {len(signals_wide)} dates")
    return signals_wide


def compute_diagnostics(market_df: pd.DataFrame, signals_df: pd.DataFrame, bucket_filter: str = None) -> pd.DataFrame:
    """
    ...
    signal_cols = [c for c in signals_df.columns if c.endswith('_signal_1') or c.endswith('_signal_2')]
...
def generate_report(results: pd.DataFrame, min_ic: float = 0.0) -> None:
    ...
    # Display table
    print("\n" + "-" * 90)
    print(f"{'Bucket':<15} {'Feature':<10} {'IC_21d':>8} {'IC_HV':>8} {'IC_LV':>8} {'Stab':>6} {'Leak':>6} {'Leaky?'}")
    print("-" * 90)
    ...
    if len(leaky) > 0:
        ...
        for _, row in leaky.iterrows():
            print(f"  ! {row['Bucket']}/{row['Feature']}: leakage_corr={row['Leakage_Corr']:.4f}")

    # Regime specialists
    print("\nRegime-specific performers:")
    hv_specialists = results[results['IC_HighVol'].abs() > results['IC_LowVol'].abs() * 1.5]
    if len(hv_specialists) > 0:
        print("  High Vol specialists:")
        for _, row in hv_specialists.head(3).iterrows():
            print(f"    {row['Bucket']}/{row['Feature']}: IC_HV={row['IC_HighVol']:.4f}, IC_LV={row['IC_LowVol']:.4f}")


def main():
    parser = argparse.ArgumentParser(description="Signal Quality Diagnostics")
    parser.add_argument("--bucket", type=str, default=None, help="Filter to specific bucket")
    parser.add_argument("--min-ic", type=float, default=0.0, help="Minimum |IC_21d| to display")
    ...
    try:
        ...
        if signals_df.empty:
            logger.error("No specialist signals found. Run generate_specialist_signals.py first.")
            return 1
        ...
        if args.output:
            ...
            output_path = PROJECT_ROOT / "docs" / "audit" / "signal_diagnostics_report.csv"
            output_path.parent.mkdir(parents=True, exist_ok=True)
            results.to_csv(output_path, index=False)
            print(f"\nResults saved to {output_path}")
