...
result = run_query(cur, "SELECT DISTINCT canonical_id FROM metadata.symbol_mapping ORDER BY canonical_id")
...
result = run_query(cur, "SELECT DISTINCT symbol FROM raw.market_futures_1d ORDER BY symbol")
if isinstance(result, list):
    actual_symbols = [r[0] for r in result]
    print(f'\nActual symbols in market_futures_1d ({len(actual_symbols)}):')
    print(f'  {actual_symbols[:20]}...' if len(actual_symbols) > 20 else f'  {actual_symbols}')
...
model_tables = [
    'cv_folds', 'garch_parameters', 'lasso_coefficients', 'meta_ensemble',
    'meta_weights', 'model_leaderboard', 'model_registry', 'oof_predictions',
    'regime_probabilities', 'shap_summary', 'shap_values'
]
...
specialist_tables = ['biofuel', 'china', 'crush', 'energy', 'fed', 'fx', 
...
# ============================================================================
print('\n' + '='*60)
print('5. RAW DATA FRESHNESS')
print('='*60)

today = datetime.now().date()
freshness_checks = [
    ('market_futures_1d', 'event_date', 'ZL daily'),
    ('cftc_cot_1w', 'event_date', 'CFTC COT'),
...
for table, date_col, desc in freshness_checks:
    result = run_query(cur, f'SELECT COUNT(*), MAX({date_col})::date FROM raw."{table}"')
    if isinstance(result, list) and result[0][1]:
        ...
        print(f'  {desc:20}: {cnt:>8,} rows, last {max_date}, {days_stale}d stale {status}')
...
# Compare to actual whitehouse_actions_event
result = run_query(cur, """
    SELECT action_type, COUNT(*), MIN(action_date), MAX(action_date)
    FROM raw.whitehouse_actions_event
    GROUP BY action_type
""")
if isinstance(result, list):
    print('\nraw.whitehouse_actions_event (REAL data):')
    for r in result:
        print(f'  {r[0]}: {r[1]} rows ({r[2]} to {r[3]})')
