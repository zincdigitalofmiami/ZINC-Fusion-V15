...
HIST_DATA_PATHS = [
    Path("/Volumes/Satechi Hub/Historical Data"),
    Path("/Volumes/Satechi Hub/ZINC-FUSION-V15/data"),
]

# Output report path
REPORT_PATH = Path("/Volumes/Satechi Hub/ZINC-FUSION-V15/data/validation_report.json")

# Date column patterns to look for
DATE_PATTERNS = [
    'date', 'as_of_date', 'timestamp', 'time', 'datetime',
    'report_date', 'trade_date', 'observation_date', 'created_at',
    'updated_at', 'period', 'year', 'month', 'day'
]
...
class DataValidator:
    ...
    def scan_directory(self, base_path: Path) -> List[Path]:
        ...
        logger.info(f"Found {len(parquet_files)} parquet files, {len(csv_files)} CSV files in {base_path}")
    ...
    def detect_duplicates(self, df: pd.DataFrame, date_cols: List[str]) -> Dict:
        ...
        if len(df) == 0:
            return {'total_rows': 0, 'duplicate_rows': 0, 'duplicate_pct': 0, 'key_columns': []}
        ...
        try:
            ...
            return {'total_rows': len(df), 'duplicate_rows': 0, 'duplicate_pct': 0, 'key_columns': key_cols}
    ...
    def validate_file(self, file_path: Path, sample_size: int = 10000) -> Dict:
        ...
        try:
            ...
            # Quality checks
            result['quality'] = {
                'empty_columns': [col for col, info in result['schema'].items()
                                 if info['null_pct'] == 100],
                'high_null_columns': [col for col, info in result['schema'].items()
                                     if 50 <= info['null_pct'] < 100],
                'constant_columns': [col for col, info in result['schema'].items()
                                    if info['unique_count'] == 1 and info['null_pct'] < 100]
            }

            # Warnings
            if result['quality']['empty_columns']:
                result['warnings'].append(f"Empty columns: {result['quality']['empty_columns']}")
            if result['duplicates']['duplicate_pct'] > 5:
                result['warnings'].append(f"High duplicate rate: {result['duplicates']['duplicate_pct']}%")
    ...
    def categorize_file(self, result: Dict) -> str:
        ...
        # Market data
        if any(x in file_name for x in ['ohlcv', 'futures', 'market', 'price', 'databento']):
        ...
        if any(x in file_name for x in ['technical', 'indicator', 'sma', 'rsi', 'macd']):
    ...
    def generate_report(self, results: List[Dict]) -> Dict:
        ...
        report = {
            ...
            'category_summary': defaultdict(lambda: {'count': 0, 'rows': 0, 'files': []}),
            'date_coverage': {},
            'schema_catalog': {},
            'issues': [],
            'files': results
            ...
            for col, date_info in r.get('date_ranges', {}).items():
                if 'min_date' in date_info and 'max_date' in date_info:
                    ...
                    if key not in report['date_coverage']:
                        ...
                        if date_info['min_date'] < report['date_coverage'][key]['min_date']:
                            report['date_coverage'][key]['min_date'] = date_info['min_date']
                        if date_info['max_date'] > report['date_coverage'][key]['max_date']:
                            report['date_coverage'][key]['max_date'] = date_info['max_date']
        ...
        report['category_summary'] = {k: dict(v) for k, v in report['category_summary'].items()}
...
def validate_single_file(file_path: str):
    ...
    for col, info in result.get('date_ranges', {}).items():
        if 'min_date' in info:
            print(f"  {col}: {info['min_date']} to {info['max_date']} ({info['count']:,} rows)")
            if info.get('gaps'):
...
def main():
    parser = argparse.ArgumentParser(description="Validate data sources before ingestion")
    parser.add_argument("--scan", action="store_true", help="Scan and validate all parquet files")
    parser.add_argument("--validate", type=str, help="Validate a single parquet file")
    parser.add_argument("--report", action="store_true", help="Show saved validation report")
