...
for year in range(2017, datetime.now().year + 1):
    CFTC_CITS_URLS[str(year)] = (
        f"https://www.cftc.gov/files/dea/history/dea_cit_txt_{year}.zip"
    )
...
def transform_cits_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "As_of_Date_In_Form_YYYY-MM-DD" in df.columns:
        ...
        df["report_date"] = pd.to_datetime(
            df["As_of_Date_In_Form_YYMMDD"].astype(str), format="%y%m%d"
        )

    df["event_date"] = df["report_date"]

    # Index trader positions (the key unique data!)
    df["index_trader_longs"] = (
        pd.to_numeric(df.get("CIT_Positions_Long_All", 0), errors="coerce")
        .fillna(0)
        .astype(int)
    )
    df["index_trader_shorts"] = (
        pd.to_numeric(df.get("CIT_Positions_Short_All", 0), errors="coerce")
        .fillna(0)
        .astype(int)
    )
    ...
    df["non_commercial_longs"] = (
        pd.to_numeric(df.get("NComm_Positions_Long_All_NoCIT", 0), errors="coerce")
        .fillna(0)
        .astype(int)
    )
    df["non_commercial_shorts"] = (
        pd.to_numeric(df.get("NComm_Positions_Short_All_NoCIT", 0), errors="coerce")
        .fillna(0)
        .astype(int)
    )
    df["non_commercial_spreads"] = (
        pd.to_numeric(df.get("NComm_Postions_Spread_All_NoCIT", 0), errors="coerce")
        .fillna(0)
        .astype(int)
    )
    ...
    # Commercial (NoCIT)
    df["commercial_longs"] = (
        pd.to_numeric(df.get("Comm_Positions_Long_All_NoCIT", 0), errors="coerce")
        .fillna(0)
        .astype(int)
    )
    df["commercial_shorts"] = (
        pd.to_numeric(df.get("Comm_Positions_Short_All_NoCIT", 0), errors="coerce")
        .fillna(0)
        .astype(int)
    )
    ...
    # Non-reportable
    df["non_reportable_longs"] = (
        pd.to_numeric(df.get("NonRept_Positions_Long_All", 0), errors="coerce")
        .fillna(0)
        .astype(int)
    )
    df["non_reportable_shorts"] = (
        pd.to_numeric(df.get("NonRept_Positions_Short_All", 0), errors="coerce")
        .fillna(0)
        .astype(int)
    )

    # Total reportable
    df["total_reportable_longs"] = (
        pd.to_numeric(df.get("Tot_Rept_Positions_Long_All", 0), errors="coerce")
        .fillna(0)
        .astype(int)
    )
    df["total_reportable_shorts"] = (
        pd.to_numeric(df.get("Tot_Rept_Positions_Short_All", 0), errors="coerce")
        .fillna(0)
        .astype(int)
    )
    ...
    df["market_participation"] = (
        pd.to_numeric(df.get("Open_Interest_All", 0), errors="coerce")
        .fillna(0)
        .astype(int)
    )
...
def upsert_cits_data(conn, df: pd.DataFrame) -> int:
    ...
    insert_sql = f"""
        INSERT INTO pos.cftc_cits_1w ({", ".join(columns)})  -- sqlref: ignore
        VALUES %s
...
def verify_data(conn):
    ...
    cur.execute(
        "SELECT COUNT(*), MIN(event_date), MAX(event_date) FROM pos.cftc_cits_1w"
    )
