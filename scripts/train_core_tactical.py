#!/usr/bin/env python3
"""
ZINC-FUSION-V15: TACTICAL Core Model Training (5d, 21d, 63d, 126d)

Trains the TACTICAL Core models using AutoGluon 1.5 TimeSeriesPredictor.
Uses FULL multi-symbol business daily data from Prisma Postgres.

TACTICAL = Lighter, faster training for operational forecasts
STRATEGIC = Full ensemble with all bells and whistles (see train_core_chronos.py)

Horizons:
    5d   - Weekly tactical
    21d  - Monthly tactical
    63d  - Quarterly tactical
    126d - 6-month tactical

Modes:
    quick - Chronos-2 only, 2 hours
    full  - Full AutoML ensemble, 6 hours

Usage:
    python scripts/train_core_tactical.py --horizon 126 --mode full
    python scripts/train_core_tactical.py --horizon all --mode full
"""
...
os.environ["PYTORCH_ENABLE_MPS_FALLBACK"] = "1"  # Fallback for unsupported MPS ops
os.environ["PYTORCH_MPS_HIGH_WATERMARK_RATIO"] = "0.0"  # Disable memory limit for MPS
    ...
    # Full mode: Complete ensemble with all models
        ...
        fit_kwargs["hyperparameters"] = {
            ...
            "Chronos2": {
                ...
                "device": "mps",  # Apple Silicon M4 Pro GPU
                "fine_tune": False,  # Disabled - LoRA crashes with 1035 features
            },
...
def train_core_chronos2(horizon: int, mode: str = "quick", dry_run: bool = False):
    ...
    try:
        ...
        model_version = f"tactical_chronos2_ag15_{mode}_h{horizon}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

        # TACTICAL models - all horizons save to tactical/ subfolder
        model_path = MODEL_PATH / f"horizon_{horizon}d" / "tactical"
