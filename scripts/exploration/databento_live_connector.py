...
def test_data_corruption() -> TestResult:
    ...
    try:
        ...
        has_errors = (
            not isinstance(invalid_data.get("open"), (int, float))
            or invalid_data.get("volume", 0) < 0
        )
        ...
        return TestResult(
            "data_corruption", False, f"Error handling invalid data: {e}", {}
        )
