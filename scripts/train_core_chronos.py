...
def load_training_data(conn, horizon: int = 5) -> pd.DataFrame:
    ...
    # Tactical vs Strategic data window
    is_tactical = horizon in TACTICAL_HORIZONS
    start_date = TACTICAL_START_DATE if is_tactical else STRATEGIC_START_DATE
    mode_label = "TACTICAL" if is_tactical else "STRATEGIC"
    freq_label = "1d"
...
def train_chronos2_model(ts_data, horizon: int, model_path: Path, mode: str = "quick"):
    ...
    # =========================================================================
    # TRAINING CONFIG - From CORE_TRAINING_SPEC_LOCKED.md
    ...
    if mode == "ultrafast":
        ...
        logger.info("   Using ULTRAFAST: Statistical models only")
    ...
    # TACTICAL (5d/21d): Chronos-Bolt + RecursiveTabular
        ...
        logger.info("   Using TACTICAL: Chronos-Bolt + RecursiveTabular")

    # STRATEGIC (63d/126d): Chronos-2 (LoRA), NO RecursiveTabular
    else:
        if horizon == 63:
            fit_kwargs["hyperparameters"] = {
                "Chronos2": {
            ...
            logger.info("   Using STRATEGIC 63d: Chronos-2 (300 steps, light LoRA)")
        else:  # 126d
            fit_kwargs["hyperparameters"] = {
                "Chronos2": {
                    "context_length": 2048,
            ...
            logger.info("   Using STRATEGIC 126d: Chronos-2 (500 steps, fuller LoRA)")
...
def train_core_chronos2(horizon: int, mode: str = "quick", dry_run: bool = False):
    ...
    try:
        ...
        # Determine tactical vs strategic
        is_tactical = horizon in TACTICAL_HORIZONS
        mode_folder = "tactical" if is_tactical else "strategic"
        model_version = f"{mode_folder}_chronos_ag15_{mode}_h{horizon}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

        # Save to correct tactical/strategic subfolder
        model_path = MODEL_PATH / f"horizon_{horizon}d" / mode_folder
