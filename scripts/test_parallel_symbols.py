...
Write to separate test tables:
- analytics.zl_price_15m_test_c (calendar)
- analytics.zl_price_15m_test_n (OI-ranked)
...
class ParallelCollector:
    ...
    def __init__(self, symbol: str, table_suffix: str):
        ...
        self.table_name = f"analytics.zl_price_15m_test_{table_suffix}"
        self.bars: List[Bar] = []
        self.running = True
    ...
    def collect_and_write(self, duration_days: int):
        ...
        try:
            for record in client:
                ...
                bucket_ts = datetime.fromtimestamp(b15 / 1000, tz=timezone.utc)
                ...
                if current_15m is None:
                    ...
                    current_bar = Bar(
                        ...
                        volume=int(record.volume) if hasattr(record, "volume") and record.volume is not None else 0
                    )
                    ...
                    current_bar.volume += int(record.volume) if hasattr(record, "volume") and record.volume is not None else 0
            ...
            conn.close()
            print(f"[{self.symbol}] Collection complete. Total bars written: {bars_written}")
...
def compare_results() -> Dict:
    ...
    common["price_diff_pct"] = abs(common["close_c"] - common["close_n"]) / common["close_c"] * 100
    common["volume_diff_pct"] = abs(common["volume_c"] - common["volume_n"]) / common[["volume_c", "volume_n"]].max(axis=1) * 100
    ...
    price_stats = {
        ...
        "bars_lt_0_1_pct_pct": (common["price_diff_pct"] < 0.1).sum() / len(common) * 100
    }
...
def main():
    ...
    if "--compare" in sys.argv:
        ...
        if "price_stats" in comparison:
            ...
            print(f"  Bars <0.1%: {ps['bars_lt_0_1_pct']} ({ps['bars_lt_0_1_pct_pct']:.1f}%)")
