    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            "memory": {
                ...
                "stable": (max(mem_values) - min(mem_values)) < 100 if mem_values else True  # <100MB variation
            },
            "cpu": {
    ...
    try:
        for record in client:
            ...
            if time.time() - last_metrics_time >= metrics_interval:
                ...
                print(f"[{datetime.now().isoformat()}] Processed {bars_processed} bars, Memory: {monitor.metrics[-1].get('memory_mb', 0):.1f}MB" if monitor.metrics else "")
...
def main():
    ...
    if "metrics_summary" in results:
        ...
        if "memory" in ms:
            mem = ms["memory"]
            print(f"  Memory: {mem.get('avg_mb', 0):.1f}MB avg ({mem.get('min_mb', 0):.1f}-{mem.get('max_mb', 0):.1f}MB)")
            print(f"    Stable: {'✓' if mem.get('stable') else '✗'}")
        
        if "cpu" in ms:
            cpu = ms["cpu"]
            print(f"  CPU: {cpu.get('avg_percent', 0):.1f}% avg ({cpu.get('min_percent', 0):.1f}-{cpu.get('max_percent', 0):.1f}%)")
            print(f"    Meets threshold (<10%): {'✓' if cpu.get('meets_threshold') else '✗'}")
