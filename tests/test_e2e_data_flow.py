...
def check_event_payload(event_data: Dict) -> Dict:
    ...
    return {
        "passed": passed,
        "message": "Event payload valid"
        if passed
        else f"Missing: {missing_fields}, Type errors: {type_errors}",
...
def check_api_response(api_url: str, timestamp: str) -> Dict:
    ...
    try:
        ...
        return {"passed": False, "message": f"API request error: {e}", "error": str(e)}
...
def check_end_to_end_latency(engine) -> Dict:
    ...
    try:
        ...
        return {
            ...
            + (" (meets threshold)" if passed else " (exceeds 2s threshold)"),
...
def main():
    ...
    if len(df) > 0:
        ...
        results["api_response"] = check_api_response(
            f"{API_BASE_URL}/api/zl/intraday?hours=24", latest_ts
        )
    else:
