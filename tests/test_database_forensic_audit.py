...
def test_no_banned_schemas(db_connection):
    ...
    """
    banned_schemas = {'raw', 'gold', 'silver', 'bronze', 'monitoring', 'specialist', 'weather', 'archive'}
    ...
    assert not violating_schemas, f"Banned schemas found in database: {violating_schemas}"
...
def test_canonical_schemas_only(db_connection):
    ...
    """
    allowed_schemas = {
        'alt', 'analytics', 'econ', 'features', 'forecasts', 'mkt', 'model', 'ops', 'pos', 'supply', 'training', 'metadata', 'vegas', 'public'
    }
    tables = get_all_tables(db_connection)
    found_schemas = {t[0] for t in tables}
    ...
    assert not extra_schemas, f"Non-canonical schemas found: {extra_schemas}. Please verify if they should be allowed."
    ...
    for schema, table in tables:
        ...
        if schema in landing_schemas:
            ...
            if 'trade_date' in columns:
                violations.append(f"{schema}.{table} uses trade_date instead of event_date")
            if 'as_of_date' in columns and schema != 'alt': # alt might use as_of_date for some things, but usually event_date
        ...
        if schema in derived_schemas:
            ...
            if 'event_date' in columns:
                violations.append(f"{schema}.{table} uses event_date instead of trade_date")
...
def test_as_of_date_audit(db_connection):
    ...
    assert not violations, f"Landing tables using as_of_date (should use event_date): {violations}"
...
def test_banned_patterns_in_table_names(db_connection):
    ...
    for schema, table in tables:
        ...
        for sym in symbols:
            if f"_{sym}_" in table.lower() or table.lower().startswith(f"{sym}_") or table.lower().endswith(f"_{sym}"):
