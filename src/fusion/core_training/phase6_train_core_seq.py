...
Training Strategy:
- Tactical horizons (5d, 21d): 7-year window, Chronos-Bolt
- Strategic horizons (63d, 126d): Full history (2000+), Chronos-2 LoRA

Key Rules:
- All features as OBSERVED covariates (not known)
- num_val_windows=3 expanding windows
- DO NOT touch model.cv_folds
- OOF predictions via predictor.backtest_predictions()
- Sequential training: 5 → 21 → 63 → 126

NORMALIZATION (LEAKAGE-SAFE):
- Core matrix stores RAW features (no global normalization)
- AutoGluon's TimeSeriesPredictor handles feature scaling internally
- Each validation window only sees data up to cutoff_date
- Scaler params are NOT shared across windows
