...
def validate_data_completeness(conn, strict: bool = True) -> ValidationResult:
    ...
    valid_sources = len(REQUIRED_DATA_SOURCES) - len(missing_sources) - len(insufficient_sources)
    is_valid = len(missing_sources) == 0 and len(insufficient_sources) == 0

    logger.info("=" * 70)
    logger.info(f"VALIDATION RESULT: {valid_sources}/{len(REQUIRED_DATA_SOURCES)} sources OK")
...
def validate_training_data(df, horizon: int, strict: bool = True) -> bool:
    ...
    symbol_cols = [c for c in cols if '_' in c and c.split('_')[1] in ['open', 'high', 'low', 'close', 'volume']]
    ...
    logger.info("=" * 70)
    logger.info(f"  Symbol features: {len(symbol_cols)} (expected ~{expected['symbol_features']})")
    logger.info(f"  Weather features: {len(weather_cols)} (expected {expected['weather_features']})")
    logger.info(f"  FX features: {len(fx_cols)} (expected {expected['fx_features']})")
    logger.info(f"  COT features: {len(cot_cols)} (expected {expected['cot_features']})")
    logger.info(f"  USDA/WASDE features: {len(usda_cols)} (expected {expected['usda_export_features'] + expected['wasde_features']})")
    logger.info(f"  RIN features: {len(rin_cols)} (expected {expected['rin_features']})")
    logger.info(f"  News features: {len(news_cols)} (expected {expected['news_features']})")
    ...
    if len(symbol_cols) < expected['symbol_features'] * 0.9:  # Allow 10% tolerance
        missing.append(f"symbol_features ({len(symbol_cols)} < {expected['symbol_features']})")
    if len(weather_cols) < expected['weather_features']:
        missing.append(f"weather_features ({len(weather_cols)} < {expected['weather_features']})")
    ...
    if len(news_cols) < expected['news_features']:
        missing.append(f"news_features ({len(news_cols)} < {expected['news_features']})")
