...
from .gs_quant_technicals import (
    ...
    exponential_moving_average as gs_ema,
    moving_average as gs_ma
)
...
class InstitutionalEliteIndicators:
    ...
    def add_hurst_exponent(self, lookback_periods: int = 100) -> pd.DataFrame:
        ...
        hurst_values = [r.hurst_exponent if r.hurst_exponent is not None else np.nan for r in results]
    ...
    def add_schaff_trend_cycle(self, cycle_periods: int = 10, fast_periods: int = 23, 
                                slow_periods: int = 50) -> pd.DataFrame:
        """
        Schaff Trend Cycle using Stock Indicators for Python (INSTITUTIONAL)
        
        Source: https://python.stockindicators.dev/indicators/Stc
        """
        results = indicators.get_stc(self.quotes, cycle_periods, fast_periods, slow_periods)
    ...
    def add_macd_gs_quant(self, fast: int = 12, slow: int = 26, signal: int = 9) -> pd.DataFrame:
        """
    ...
    def add_bollinger_bands_gs_quant(self, window: int = 20, k: float = 2.0) -> pd.DataFrame:
        """
