...
- Equity: DJT, FXI, KWEB (probability proxies)
...
Usage:
    ...
    engine = TrumpEffectFeatureEngine(fred_df, equity_df)
...
class ProbabilityProxies:
    """Market-implied probability proxies from Equity tickers."""
...
# =============================================================================
# PROBABILITY PROXIES (Equity-derived)
# =============================================================================
...
def calculate_probability_proxies(
    ...
    """
    Calculate market-implied probability proxies from Equity tickers.

    Required columns in equity_df:
        - as_of_date, ticker, close, adj_close

    Expected tickers: DJT, FXI, KWEB

    Args:
        equity_df: Equity price data
        as_of_date: Reference date
    ...
    pivot = equity_df.pivot(index="as_of_date", columns="ticker", values="adj_close")
...
class TrumpEffectFeatureEngine:
    ...
    Combines FRED series, Equity proxies, and event data into
    ...
    def __init__(
        ...
        Args:
            ...
            equity_df: Equity prices (as_of_date, ticker, close, adj_close)
    ...
    def compute_features_for_date(
        ...
        # Probability proxies (Equity)
        if self.equity_df is not None:
            proxies = calculate_probability_proxies(self.equity_df, as_of_date)
...
def quick_trump_effect_summary(
    ...
    if equity_df is not None:
        proxies = calculate_probability_proxies(equity_df, as_of_date)
