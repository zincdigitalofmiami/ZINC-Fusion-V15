...
try:
    ...
    def rolling_apply(
        s: pd.Series, offset: pd.DateOffset, function: Callable[[np.ndarray], float]
    ) -> pd.Series:
        if isinstance(s.index, pd.DatetimeIndex):
            values = [
                function(s.loc[(s.index > (idx - offset)) & (s.index <= idx)])
                for idx in s.index
            ]
        else:
            values = [
                function(s.loc[(s.index > (idx - offset).date()) & (s.index <= idx)])
                for idx in s.index
            ]
...
class Window:
    ...
    **Examples**
    ...
    def __init__(
        self, w: Union[int, str, None] = None, r: Union[int, str, None] = None
    ):
        self.w = w
...
def apply_ramp(x: pd.Series, window: Window) -> pd.Series:
    ...
    if isinstance(window.w, int) and window.w > len(
        x
    ):  # does not restrict window size when it is a DataOffset
...
def get_dataset_with_many_assets(
    ...
    **kwargs,
    ...
    tasks = [
        partial(
            ds.get_data,
            assetId=assets[i : i + batch_limit],
            start=start,
            end=end,
            return_type=None,
            **kwargs,
    ...
    if method_name and len(offset.kwds) == 1:
        ...
        if freq in fixed:
            ...
            if np.issubdtype(s.index, np.datetime64):
                ...
                return pd.Series(
                    _pandas_roll(t, window_str, method_name), index=s.index
                )
