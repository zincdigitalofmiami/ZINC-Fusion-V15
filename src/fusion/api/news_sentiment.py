...
def analyze_articles(articles: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    summary = {
        ...
        "direction_counts": {
            ...
            "uncertain": sum(1 for a in analyzed if a["overall_direction"] == "uncertain"),
        },
        "bucket_counts": {},
    }
...
def get_policy_sentiment(limit: int = 30) -> List[Dict[str, Any]]:
    ...
    try:
        exists = (
            conn.execute(
                ...
                WHERE table_schema = 'features' AND table_name = 'policy_sentiment_1d'
                """
            ).fetchone()[0]
            > 0
        )
    ...
    data = []
    for row in rows:
        record = {columns[idx]: _serialize_value(value) for idx, value in enumerate(row)}
        data.append(record)
    return data
