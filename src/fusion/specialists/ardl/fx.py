...
class FxSignalGenerator(BaseSignalGenerator):
    ...
    # =========================================================================
    ALL_FX_PAIRS: ClassVar[dict[str, dict[str, float | str]]] = {
    ...
    TRADE_WEIGHTED_INDICES: ClassVar[dict[str, str]] = {
    ...
    def _compute_fx_correlations(self, data: pd.DataFrame) -> dict[str, pd.DataFrame]:
        ...
        zl_correlations = {}
        available_fx = [col for col in self.ALL_FX_PAIRS if col in data.columns]
    ...
    def _compute_dynamic_weights(
        ...
        dynamic_weights = {}
        available_fx = [col for col in self.base_trade_weights if col in data.columns]
    ...
    def _compute_proxy_carry_signal(
        ...
        if (
            "fred_t10y2y" not in df.columns
            and "fred_dgs10" in df.columns
            and "fred_dgs2" in df.columns
        ):
