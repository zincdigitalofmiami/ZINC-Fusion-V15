...
class BaseSignalGenerator(ABC):
    ...
    def _required_features(self) -> List[str]:
        ...
        ordered = self.config.primary_features + self.config.secondary_features + self.config.critical_features
    ...
    def add_all_elite_indicators(
        ...
        results[f"{prefix}_cumulative_rsi"] = results[f"{prefix}_rsi_2"].rolling(3).sum()
        ...
        roc_pct = roc_1d.rolling(100, min_periods=20).apply(
            lambda x: (x.iloc[-1] > x.iloc[:-1]).sum() / len(x.iloc[:-1]) * 100 if len(x) > 1 else 50, raw=False
        )
        results[f"{prefix}_connors_rsi"] = (rsi_3.fillna(50) + rsi_streak.fillna(50) + roc_pct.fillna(50)) / 3
        ...
        results[f"{prefix}_macd_signal"] = results[f"{prefix}_macd"].ewm(span=9, adjust=False).mean()
        results[f"{prefix}_macd_hist"] = results[f"{prefix}_macd"] - results[f"{prefix}_macd_signal"]
        ...
        results[f"{prefix}_macd_fast_signal"] = results[f"{prefix}_macd_fast"].ewm(span=4, adjust=False).mean()
        results[f"{prefix}_macd_fast_hist"] = results[f"{prefix}_macd_fast"] - results[f"{prefix}_macd_fast_signal"]
        ...
        # HMA(20)
        wma_half = close.rolling(10).apply(lambda x: np.average(x, weights=range(1, len(x)+1)), raw=True)
        wma_full = close.rolling(20).apply(lambda x: np.average(x, weights=range(1, len(x)+1)), raw=True)
        raw_hma = 2 * wma_half - wma_full
        results[f"{prefix}_hma_20"] = raw_hma.rolling(4).apply(lambda x: np.average(x, weights=range(1, len(x)+1)), raw=True)
        ...
        for mcg_period in [14, 100]:
            ...
            for i in range(1, len(close)):
                if pd.isna(close.iloc[i]) or pd.isna(md.iloc[i-1]) or md.iloc[i-1] == 0:
                    ...
                    md.iloc[i] = md.iloc[i-1] + (close.iloc[i] - md.iloc[i-1]) / k if k > 0 else md.iloc[i-1]
        ...
        for window in [5, 10, 21, 63, 126, 252]:
            ...
            results[f"{prefix}_zscore_{window}d"] = (close - mean) / std.replace(0, np.nan)

        # =====================================================================
        # 7. MOMENTUM at multiple periods
        # =====================================================================
        for period in [1, 5, 10, 21, 63, 126]:
            results[f"{prefix}_mom_{period}d"] = close.pct_change(period, fill_method=None) * 100

        # =====================================================================
        # 8. VOLATILITY - ATR, Garman-Klass, Yang-Zhang, Bollinger
        # =====================================================================
        if high_col and low_col and high_col in data.columns and low_col in data.columns:
            ...
            results[f"{prefix}_atr_ratio"] = results[f"{prefix}_atr_10"] / results[f"{prefix}_atr_50"]
            ...
            if open_col and open_col in data.columns:
                ...
                log_co = np.where((close == open_p) | (co_ratio <= 0) | (open_p == 0), 0.0, np.log(co_ratio))
                gk_daily = np.maximum(0.5 * log_hl**2 - 0.386 * log_co**2, 0.0)
                gk_rolling = pd.Series(gk_daily, index=data.index).rolling(20).mean()
                results[f"{prefix}_garman_klass"] = np.sqrt(np.maximum(gk_rolling, 0.0) * 252) * 100
            ...
            for cci_period in [14, 50]:
                ...
                mad = typical.rolling(cci_period).apply(lambda x: np.abs(x - x.mean()).mean(), raw=True)
                results[f"{prefix}_cci_{cci_period}"] = (typical - sma_tp) / (0.015 * mad)
            ...
            stoch1 = 100 * (macd_stc - lowest_macd) / (highest_macd - lowest_macd).replace(0, np.nan)
        ...
        if volume_col and volume_col in data.columns:
            ...
            results[f"{prefix}_volume_zscore"] = (volume - vol_mean) / vol_std.replace(0, np.nan)
            ...
            if high_col and low_col:
                ...
                results[f"{prefix}_cmf_21"] = np.where(vol_sum == 0, 0.0, mfv_sum / vol_sum)

            # Elder Force Index
            results[f"{prefix}_elder_force"] = (close.diff() * volume).ewm(span=13).mean()
        ...
        # =====================================================================
        for ma_col in ["sma_10", "sma_20", "sma_50", "sma_100", "sma_200", "ema_10", "ema_20", "ema_50"]:
            full_col = f"{prefix}_{ma_col}"
            if full_col in results:
                results[f"{prefix}_dist_{ma_col}"] = (close - results[full_col]) / results[full_col] * 100

        # =====================================================================
        # 16. RATE OF CHANGE
        # =====================================================================
        for period in [5, 10, 21]:
            results[f"{prefix}_roc_{period}"] = ((close - close.shift(period)) / close.shift(period)) * 100
        ...
        if high_col and low_col and high_col in data.columns and low_col in data.columns:
            ...
            results[f"{prefix}_williams_r"] = -100 * (highest_high - close) / (highest_high - lowest_low).replace(0, np.nan)
    ...
    def get_all_elite_indicator_names(self, prefix: str) -> List[str]:
        ...
        return [
            ...
            f"{prefix}_hurst",
            # RSI variants
            f"{prefix}_rsi_2", f"{prefix}_rsi_14", f"{prefix}_cumulative_rsi", f"{prefix}_connors_rsi",
            # MACD
            f"{prefix}_macd", f"{prefix}_macd_signal", f"{prefix}_macd_hist",
            f"{prefix}_macd_fast", f"{prefix}_macd_fast_signal", f"{prefix}_macd_fast_hist",
            # Moving averages
            f"{prefix}_sma_10", f"{prefix}_sma_20", f"{prefix}_sma_50", f"{prefix}_sma_100", f"{prefix}_sma_200",
            f"{prefix}_ema_10", f"{prefix}_ema_20", f"{prefix}_ema_50",
            f"{prefix}_kama_10", f"{prefix}_hma_20", f"{prefix}_alma_50",
            f"{prefix}_mcginley_14", f"{prefix}_mcginley_100",
            # Z-scores
            f"{prefix}_zscore_5d", f"{prefix}_zscore_10d", f"{prefix}_zscore_21d",
            f"{prefix}_zscore_63d", f"{prefix}_zscore_126d", f"{prefix}_zscore_252d",
            # Momentum
            f"{prefix}_mom_1d", f"{prefix}_mom_5d", f"{prefix}_mom_10d",
            f"{prefix}_mom_21d", f"{prefix}_mom_63d", f"{prefix}_mom_126d",
            # Volatility
            f"{prefix}_atr_10", f"{prefix}_atr_14", f"{prefix}_atr_50", f"{prefix}_atr_ratio",
            f"{prefix}_garman_klass", f"{prefix}_yang_zhang",
            f"{prefix}_bb_upper", f"{prefix}_bb_lower", f"{prefix}_bb_pct_b", f"{prefix}_bb_width",
            # Oscillators
            f"{prefix}_fisher", f"{prefix}_fisher_signal",
            f"{prefix}_ttm_squeeze_on", f"{prefix}_ttm_squeeze_mom",
            f"{prefix}_cci_14", f"{prefix}_cci_50",
            f"{prefix}_schaff",
            f"{prefix}_rvi", f"{prefix}_rvi_signal",
            # Volume
            f"{prefix}_volume_zscore", f"{prefix}_cmf_21", f"{prefix}_elder_force", f"{prefix}_obv",
            # Distance from MAs
            f"{prefix}_dist_sma_10", f"{prefix}_dist_sma_20", f"{prefix}_dist_sma_50",
            f"{prefix}_dist_sma_100", f"{prefix}_dist_sma_200",
            f"{prefix}_dist_ema_10", f"{prefix}_dist_ema_20", f"{prefix}_dist_ema_50",
            # ROC
            f"{prefix}_roc_5", f"{prefix}_roc_10", f"{prefix}_roc_21",
            # Stochastic
            f"{prefix}_stoch_k_14", f"{prefix}_stoch_d_14",
            f"{prefix}_stoch_k_21", f"{prefix}_stoch_d_21",
            # Williams %R
            f"{prefix}_williams_r",
            # ADX
            f"{prefix}_adx", f"{prefix}_plus_di", f"{prefix}_minus_di",
            # Autocorrelation
            f"{prefix}_autocorr_21d", f"{prefix}_autocorr_63d", f"{prefix}_autocorr_126d",
        ]
