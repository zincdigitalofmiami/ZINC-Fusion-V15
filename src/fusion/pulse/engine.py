...
from .schema import IntelDrop, Domain, Horizon, PULSE_JSON_SCHEMA
...
class PulseEngine:
    ...
    def build_prompt(
        ...
        prompt = prompt.replace("{{SIGNAL_SNAPSHOT_JSON}}", json.dumps(signal_snapshot, indent=2))
        prompt = prompt.replace("{{EVENT_STREAM}}", "\n".join(event_stream))
        prompt = prompt.replace("{{RECEIPT_IDS}}", json.dumps(receipt_ids))
        prompt = prompt.replace("{{BENCHMARK_SUMMARIES}}", json.dumps(benchmark_summaries, indent=2))
...
EXAMPLE_SIGNAL_SNAPSHOT = {
    ...
    "BIOFUEL": {
        ...
        "renewable_diesel_margin": {"value": 0.85, "unit": "$/gal", "trend": "improving"},
