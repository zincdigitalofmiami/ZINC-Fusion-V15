...
def validate_pulse(pulse_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    if pulse_data.get("schema_version") != "pulse.v1":
        errors.append(
            f"Invalid schema_version: {pulse_data.get('schema_version')} (expected pulse.v1)"
        )
    ...
    if pulse_data.get("domain") not in valid_domains:
        errors.append(
            f"Invalid domain: {pulse_data.get('domain')} (expected one of {valid_domains})"
        )
    ...
    for i, h in enumerate(horizons):
        if i < len(expected_horizons):
            if h.get("horizon") != expected_horizons[i]:
                errors.append(
                    f"Horizon {i} should be {expected_horizons[i]}, got {h.get('horizon')}"
                )
        ...
        if abs(weight_sum - 1.0) > 0.01:
            errors.append(
                f"Horizon {h.get('horizon')}: driver_weights sum to {weight_sum:.3f}, must be 1.0 ± 0.01"
            )
        ...
        if not 0 <= edge <= 1:
            errors.append(
                f"Horizon {h.get('horizon')}: edge {edge} must be between 0 and 1"
            )
        ...
        if direction not in [-1, 0, 1]:
            errors.append(
                f"Horizon {h.get('horizon')}: direction {direction} must be -1, 0, or 1"
            )
        ...
        for driver in h.get("top_drivers", []):
            ...
            if not 0 <= driver.get("weight", 0) <= 1:
                errors.append(
                    f"Driver {driver.get('driver_id')}: weight must be between 0 and 1"
                )
