...
Checks landing tables (mkt/econ/alt/pos/supply) for stale data based on
expected update frequency. Alerts when tables haven't received new data
within their expected window.
...
# Landing schemas to monitor
LANDING_SCHEMAS: List[str] = ["mkt", "econ", "alt", "pos", "supply"]
...
class FreshnessMonitor:
    """Monitors data freshness across landing tables (mkt/econ/alt/pos/supply)."""
    ...
    def _get_landing_tables(self) -> List[tuple]:
    ...
    def _check_table_freshness(self, schema: str, table_name: str) -> TableFreshness:
        """Check freshness of a single table."""
        cadence = self._get_cadence(table_name)
        threshold = FRESHNESS_THRESHOLDS.get(cadence)

        # Use event_date for landing tables (institutional standard)
        time_column = "event_date" if schema in LANDING_SCHEMAS else "knowledge_time"

        with self.conn.cursor() as cur:
            # Get row count and latest time
            cur.execute(
                f"""
                SELECT
                    COUNT(*) as row_count,
    ...
    def check_all(self) -> bool:
        """Check freshness of all landing tables."""
    ...
    def print_report(self):
        ...
        for r in recent:
            kt_str = (
                r.latest_knowledge_time.strftime("%Y-%m-%d %H:%M:%S")
                if r.latest_knowledge_time
                else "Never"
            )
