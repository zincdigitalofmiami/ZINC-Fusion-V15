...
HIST_DATA = Path(os.getenv("HISTORICAL_DATA_PATH", "/tmp/historical_data"))
...
for file_info in files:
    ...
    schema_groups[schema].append(
        {**file_info, "target_schema": schema, "target_table": table}
    )
...
for schema in sorted(schema_groups.keys()):
    ...
    print(f"\n{'='*80}")
    print(
        f"SCHEMA: {schema.upper()} ({len(files_in_schema)} files, {total_size:.2f} MB)"
    )
    print("=" * 80)

    for f in sorted(files_in_schema, key=lambda x: x["size_mb"], reverse=True):
        print(
            f"{f['size_mb']:>8.2f} MB  {f['filename']:50} → {f['target_schema']}.{f['target_table']}"
        )
