...
function DriverCard({ label, data, metrics, loading }: DriverCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Level */}
      <div
        className={`text-lg font-medium mt-1 mb-4 ${loading ? "text-slate-600" : getScoreTextColor(score)}`}
      >
        {loading ? "..." : level}
      </div>
      ...
      {expanded && wh && (
        <div className="mt-4 w-full text-left space-y-3 animate-in slide-in-from-top-2 duration-200">
          <div
            className="text-sm text-slate-300 leading-relaxed border-l-2 pl-3"
          ...
          <div className="space-y-2 pt-1">
            ...
            <IntelSection
              ...
              content={wh.investorSentiment}
            />
            <IntelSection
              title="Near-Term Outlook"
              content={wh.nearTermOutlook}
            />
          </div>
  ...
  return (
    <div className="mt-6 border-t border-slate-800 pt-6">
      ...
      >
        <span>{expanded ? "▼" : "▶"}</span>
        <span>
          {expanded ? "Hide Full Analysis" : "Show Full Market Analysis"}
        </span>
    ...
    <div className="w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <DriverCard
          ...
          metrics={[
            {
              key: "board_crush_value",
              label: "Crush Margin",
              format: (v) => (v ? `$${v.toFixed(2)}/bu` : "--"),
            },
            {
              key: "oil_share_value",
              label: "Oil Value Share",
              format: (v) => (v !== null ? `${v?.toFixed(1)}%` : "--"),
            },
            {
              key: "oil_share_5d_change",
              label: "5-Day Change",
              format: (v) =>
        ...
        <DriverCard
          ...
          metrics={[
            {
              key: "cny_rate",
              label: "Yuan Rate (CNY/USD)",
              format: (v) => v?.toFixed(2) ?? "--",
            },
            {
              key: "soy_china_news_count",
              label: "China/Soy Headlines",
        ...
        <div className="mt-6 flex items-center justify-between text-sm text-slate-500 px-2">
          <div>
            ...
            <span
              className="font-mono"
              style={{
                color: getScoreColor(data.summary.average_pressure).stroke,
              }}
            >
          ...
          <div>
            ...
            <span className="text-slate-300">
              {mapDriverName(data.summary.highest_pressure?.name ?? "")}
            </span>{" "}
            (
            <span
              className="font-mono"
              style={{
                color: getScoreColor(data.summary.highest_pressure?.score ?? 0)
                  .stroke,
              }}
            >
      ...
      {drivers.map((d) => (
        <div key={d.label} className="flex items-center gap-1.5">
          <div
            ...
            style={{ backgroundColor: getScoreColor(d.score ?? 0).stroke }}
          />
          ...
          <span
            ...
            style={{
              color:
                d.score != null ? getScoreColor(d.score).stroke : "#64748b",
            }}
          >
