/**
 * White House News RSS Bronze Ingestion
 * 
 * BRONZE CONTRACT COMPLIANT
 * SOURCE: https://www.whitehouse.gov/news/feed/
 * Tags: trump_effect
 * 
...
export const whitehouseDaily = inngest.createFunction(
  { id: "whitehouse-daily", name: "White House News RSS Bronze Ingestion", retries: 3 },
  ...
  async ({ step, logger }) => {
    ...
    try {
      runId = await step.run("create-ingest-run", () => createIngestRun(client, "whitehouse-daily"));
      logger.info(`Started ingest run: ${runId}`);

      const items = await step.run("fetch-rss", async () => {
        const response = await fetch("https://www.whitehouse.gov/news/feed/", {
          headers: { "User-Agent": "ZINC-Fusion/1.0" }
        });
        if (!response.ok) throw new Error(`White House RSS error: ${response.status}`);
      ...
      logger.info(`Fetched ${Array.isArray(items) ? items.length : 1} items from White House RSS`);
      ...
      logger.error(`White House RSS ingestion failed: ${msg}`);
      throw error;
    } finally {
      client.release();
    }
