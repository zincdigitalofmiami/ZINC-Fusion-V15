...
export const mpobPalmMonthly = inngest.createFunction(
  ...
  async ({ step, logger }) => {
    ...
    try {
      const data = await step.run("fetch-usda-psd", () => fetchMalaysiaPalmProduction());
      logger.info(`Fetched ${data.length} Malaysia palm records`);
      
      for (const record of data) {
        const reportMonth = new Date(`${record.marketYear.split('/')[0]}-10-01`); // October = palm oil year start
