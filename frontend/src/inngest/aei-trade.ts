...
export const aeiTradeDaily = inngest.createFunction(
  { id: "aei-trade-daily", name: "AEI Trade Policy RSS Data Ingestion", retries: 3, concurrency: [DB_CONCURRENCY, { limit: 1 }] },
  { cron: "16 */8 * * *" }, // Every 8 hours at :16 UTC
  async ({ step, logger }) => {
    ...
    try {
      ...
      for (const item of itemArray) {
        ...
        const outcome = await step.run(`ingest-${stepId}`, async () => {
          ...
          if (await hashExists(client, "alt.policy_news_event", rowHash)) {
          ...
          void (item.category ? (Array.isArray(item.category) ? item.category : [item.category]) : []);
