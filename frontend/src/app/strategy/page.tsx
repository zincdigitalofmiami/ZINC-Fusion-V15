...
export default function StrategyPage() {
  ...
  const postureKey = brief
    ? brief.overrideReason
      ? brief.recommendation === "LOCK IN COVERAGE"
        ? "LOCK IN COVERAGE:OVERRIDE"
        : brief.recommendation === "WAIT"
          ? "WAIT:OVERRIDE"
          : brief.recommendation
      : brief.recommendation
    : "CHECK DATA";
  const posture = brief
    ? (POSTURE_MAP[postureKey] ?? POSTURE_MAP["CHECK DATA"])
    : null;
  ...
  const confidence = brief
    ? (() => {
        ...
        const driverPct =
          totalDrivers > 0 ? (liveDrivers / totalDrivers) * 100 : 0;
        // Weight: 60% driver coverage, 40% forecast availability
        return Math.round(
          driverPct * 0.6 + (brief.forecastsAvailable ? 40 : 0),
        );
      })()
    : 0;
  ...
  const actionItems = brief
    ? (() => {
        ...
        if (
          brief.recommendation === "LOCK IN COVERAGE" ||
          brief.recommendation === "BUY NOW"
        ) {
          const price =
            typeof brief.price?.current === "number"
              ? brief.price.current.toFixed(2)
              : "--";
          items.push({
            title: "Lock In Coverage",
            detail: `Price at $${price}`,
            primary: true,
          });
        } else if (brief.recommendation === "WAIT") {
          items.push({
            title: "Hold Off Buying",
            detail: "Headwinds detected",
            primary: true,
          });
        } else if (brief.recommendation === "NORMAL SCHEDULE") {
          items.push({
            title: "Normal Schedule",
            detail: "No urgency — follow standard plan",
            primary: true,
          });
        } else {
          items.push({
        ...
        const topDriver = brief.drivers.reduce(
  ...
  const riskCards =
    brief?.keyRisks.slice(0, 3).map((risk, i) => {
  ...
  const waterfallFactors = brief
    ? (() => {
        ...
        const absCenteredSum = centered.reduce(
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-slate-200 p-3 pt-24 md:p-6 md:pt-36 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="col-span-2 relative group overflow-hidden bg-[#0a0a0a] border border-white/5 rounded-xl p-6">
          <div
            className={`absolute inset-0 bg-gradient-to-r ${posture?.gradient ?? "from-cyan-500/5"} to-transparent pointer-events-none`}
          />
          ...
          {loading && !brief ? (
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <h2
                  className={`text-5xl font-bold tracking-tight mb-2 ${posture.color}`}
                >
                  {posture.label}
                </h2>
                <p
                  className={`max-w-md text-sm leading-relaxed ${brief.overrideReason ? "text-red-400/80 font-medium" : "text-slate-500"}`}
                >
              ...
              <div className="flex flex-col items-end gap-3 p-4 bg-black/30 rounded-lg border border-white/5">
                <div className="text-right">
                  <div
                    className={`text-2xl font-bold ${confidence >= 60 ? "text-emerald-400" : confidence >= 40 ? "text-amber-400" : "text-red-400"}`}
                  >
                    {confidence}%
                  </div>
                    ...
                    <>
                      <div
                        className={`text-2xl font-bold ${
                          expReturn.direction === "UP"
                            ? "text-cyan-400"
                            : expReturn.direction === "DOWN"
                              ? "text-red-400"
                              : "text-slate-400"
                        }`}
                      >
          ...
          {loading && !brief ? (
            ...
            <div className="space-y-2">
              {actionItems.map((item, i) => (
                ...
                >
                  <span
                    className={`text-sm font-bold ${item.primary ? "text-cyan-400" : "text-slate-500"}`}
                  >
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <div>
                    <div
                      className={`text-sm font-bold ${item.primary ? "text-white" : "text-slate-300"}`}
                    >
                      {item.title}
                    </div>
          ...
          {brief.drivers.map((driver) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`text-2xl font-bold font-mono mb-2 ${scoreColor}`}
                >
                ...
                <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden mb-2">
                  <div
                    ...
                    style={{
                      width: `${driver.source === "unavailable" ? 0 : driver.score}%`,
                    }}
                  />
                </div>
                ...
                {driver.source === "stale" && driver.asOfDate && (
                  <div className="text-[9px] text-amber-500/50 mt-1 font-mono">
                    {Math.floor(
                      (Date.now() - new Date(driver.asOfDate).getTime()) /
                        86400000,
                    )}
                    d ago
                  </div>
        ...
        <div className="mb-8">
          ...
          <div className="space-y-1.5">
            {brief.eventPulse.recentEvents.slice(0, 5).map((event, i) => (
              <div
                key={i}
                className="flex items-start gap-3 p-3 bg-[#0a0a0a] border border-white/5 rounded-lg hover:border-white/10 transition-colors"
              >
                {/* Sentiment dot */}
                <div
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] font-mono text-slate-600">
                      {event.hoursAgo <= 24
                        ...
                        : `${Math.round(event.hoursAgo / 24)}d ago`}
                    </span>
                    <span
                      className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${
                        event.source === "Federal Register"
                          ? "bg-blue-500/10 text-blue-400"
                          : event.source === "ProFarmer"
                            ? "bg-amber-500/10 text-amber-400"
                  ...
                  {fc.source === "model" && typeof fc.targetMid === "number" ? (
                    <>
                      ...
                      <div className="flex items-center gap-1.5 mb-3">
                        ...
                        <span
                          className={`text-sm font-bold font-mono ${dirColor}`}
                        >
      ...
      <div className="mb-8">
        ...
        <div className="relative w-full h-[300px] md:h-[500px] bg-[#0a0a0a] border border-white/5 rounded-xl overflow-hidden">
          ...
          <FusionBrain
            drivers={brief?.drivers}
            correlations={brief?.correlations}
          />
        </div>
      </div>
          ...
          >
            ...
            {riskCards.map((card, i) => {
              ...
              return (
                <div
                  key={`risk-${i}`}
                  className={`p-4 rounded-xl border ${card.colors.border} ${card.colors.bg}`}
                >
                  <div className="flex items-center gap-2 mb-2">
                    <IconComponent size={14} className={card.colors.text} />
                    <span className={`text-xs font-bold ${card.colors.text}`}>
                      {card.label}
                    </span>
                  </div>
                  <p
                    className={`text-[11px] ${card.colors.textMuted} leading-relaxed`}
                  >
                    {card.text}
                  </p>
                </div>
            ...
            {brief.keyPositives
              .slice(0, Math.max(1, 3 - riskCards.length))
              .map((positive, i) => (
                <div
                  key={`pos-${i}`}
