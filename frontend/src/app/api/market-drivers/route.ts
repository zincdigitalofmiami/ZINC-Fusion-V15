...
const TERM = { HEALTHY_CONTANGO: 0.85, NORMAL_CONTANGO: 0.92, FLAT: 1.00, BACKWARDATION: 1.05, SEVERE: 1.15 }
...
const VIX_ZL_CORR = { HIGH: 0.50, MODERATE: 0.30, LOW: 0.10, NEGATIVE: -0.10 }
...
const OIL_SHARE = { VERY_LOW: 0.42, LOW: 0.45, NEUTRAL_LOW: 0.47, NEUTRAL_HIGH: 0.49, HIGH: 0.51, VERY_HIGH: 0.54 }
...
function scoreTermStructure(vix: number, vix3m: number | null): { adj: number; desc: string } {
  if (!vix3m || vix3m === 0) return { adj: 0, desc: 'No term structure data' }
  const ratio = vix / vix3m
  if (ratio < TERM.HEALTHY_CONTANGO) return { adj: -15, desc: 'Steep contango - very orderly' }
  if (ratio < TERM.NORMAL_CONTANGO) return { adj: -8, desc: 'Normal contango' }
  if (ratio < TERM.FLAT) return { adj: -3, desc: 'Mild contango' }
  if (ratio < TERM.BACKWARDATION) return { adj: 5, desc: 'Flat to slight backwardation' }
  if (ratio < TERM.SEVERE) return { adj: 15, desc: 'Backwardation - near-term stress' }
...
function scoreVixZlCorrelation(corr: number | null): { adj: number; desc: string } {
  ...
  if (corr >= VIX_ZL_CORR.HIGH) {
    ...
    return { adj, desc: `High VIX-ZL transmission (${corr.toFixed(2)}) - risk-off hitting soy` }
  }
  if (corr >= VIX_ZL_CORR.MODERATE) {
    const pct = (corr - VIX_ZL_CORR.MODERATE) / (VIX_ZL_CORR.HIGH - VIX_ZL_CORR.MODERATE)
    return { adj: 5 + pct * 10, desc: `Moderate VIX-ZL correlation (${corr.toFixed(2)})` }
  }
  if (corr >= VIX_ZL_CORR.LOW) {
    return { adj: 0, desc: `Low VIX-ZL correlation (${corr.toFixed(2)}) - fundamentals driving` }
  }
  if (corr >= VIX_ZL_CORR.NEGATIVE) {
    return { adj: -5, desc: `Minimal VIX-ZL link (${corr.toFixed(2)}) - ZL independent` }
  }
  return { adj: -10, desc: `Negative VIX-ZL (${corr.toFixed(2)}) - ZL acting as hedge` }
}

function scoreHedgeSentiment(count: number): { adj: number; desc: string } {
  if (count >= 15) return { adj: 18, desc: `Heavy hedge focus (${count} articles) - farmer stress elevated` }
  if (count >= 8) return { adj: 12, desc: `Elevated hedge discussion (${count} articles)` }
  if (count >= 4) return { adj: 5, desc: `Normal hedge coverage (${count} articles)` }
  if (count >= 1) return { adj: 0, desc: `Light hedge mentions (${count} articles)` }
...
function calculateVixStress(
...
): { score: number; level: string; regime: string; headline: string; components: VixComponents } {
  ...
  if (realizedVol !== null) {
    ...
    else if (realizedVol < 0.50) rvAdj = 15
    else rvAdj = 20
  }
  ...
  return {
    ...
    components: {
      ...
      vix_zl_correlation: vixZlCorr ? Math.round(vixZlCorr * 1000) / 1000 : null,
...
function scoreBoardCrush(crush: number): { score: number; regime: string } {
  ...
  return { score: Math.max(5, 20 - Math.min(15, excess * 15)), regime: 'exceptional_margins' }
}

function scoreOilShareLevel(share: number | null): { adj: number; desc: string } {
  if (share === null) return { adj: 0, desc: 'No oil share data' }
  if (share < OIL_SHARE.VERY_LOW) return { adj: 20, desc: 'Oil severely undervalued (meal driving crush)' }
  if (share < OIL_SHARE.LOW) return { adj: 12, desc: 'Oil weak relative to meal' }
  if (share < OIL_SHARE.NEUTRAL_LOW) return { adj: 5, desc: 'Oil slightly below average' }
  if (share < OIL_SHARE.NEUTRAL_HIGH) return { adj: 0, desc: 'Oil share in normal range' }
  if (share < OIL_SHARE.HIGH) return { adj: -5, desc: 'Oil slightly above average' }
  if (share < OIL_SHARE.VERY_HIGH) return { adj: -10, desc: 'Oil commanding premium' }
  return { adj: -15, desc: 'Oil share extremely elevated' }
}

function scoreOilShareTrend(change5d: number | null): { adj: number; desc: string } {
...
function calculateCrushPressure(
  ...
  specialistSignal: number | null
): { score: number; level: string; regime: string; headline: string; components: CrushComponents } {
  ...
  score += (oilShareAdj * 0.20 / 0.45)
  score += (trendAdj * 0.20 / 0.45)
  ...
  return {
    ...
    components: {
      ...
      oil_share_5d_change: oilShareChange ? Math.round(oilShareChange * 1000) / 10 : null,  // As percentage
...
function scoreFxiPerformance(change20d: number, change5d: number): { score: number; desc: string } {
  let base: number, desc: string
  if (change20d <= -0.15) { base = 90; desc = 'China equities in freefall' }
  else if (change20d <= -0.10) { base = 80; desc = 'China equities severely weak' }
  else if (change20d <= -0.05) { base = 65; desc = 'China equities under pressure' }
  else if (change20d <= -0.02) { base = 55; desc = 'China equities soft' }
  else if (change20d <= 0.02) { base = 45; desc = 'China equities stable' }
  else if (change20d <= 0.05) { base = 35; desc = 'China equities firming' }
  else if (change20d <= 0.10) { base = 25; desc = 'China equities rallying' }
  else { base = 20; desc = 'China equities surging' }

  // Short-term momentum modifier
  if (change5d < -0.05) { base = Math.min(100, base + 10); desc += ' (accelerating weakness)' }
  else if (change5d > 0.05) { base = Math.max(0, base - 5); desc += ' (near-term bounce)' }

  return { score: base, desc }
}

function scoreCnyLevel(rate: number, change20d: number | null): { score: number; desc: string } {
  let levelScore: number, levelDesc: string
  if (rate < CNY.STRONG) { levelScore = 25; levelDesc = 'Yuan strong' }
  else if (rate < CNY.NORMAL) { levelScore = 35; levelDesc = 'Yuan stable' }
  else if (rate < CNY.WEAK) { levelScore = 50; levelDesc = 'Yuan slightly weak' }
  else if (rate < CNY.STRESS) { levelScore = 65; levelDesc = 'Yuan weak' }
  else if (rate < CNY.CRISIS) { levelScore = 80; levelDesc = 'Yuan under pressure' }
  else { levelScore = 90; levelDesc = 'Yuan crisis level' }

  // Rate of change modifier
  let rocAdj = 0, rocDesc = ''
  if (change20d !== null) {
    if (change20d >= 0.02) { rocAdj = 15; rocDesc = ', devaluing rapidly' }
    else if (change20d >= 0.01) { rocAdj = 8; rocDesc = ', weakening' }
    else if (change20d <= -0.02) { rocAdj = -15; rocDesc = ', strengthening rapidly' }
    else if (change20d <= -0.01) { rocAdj = -8; rocDesc = ', firming' }
  }

  return { score: Math.max(0, Math.min(100, levelScore + rocAdj)), desc: levelDesc + rocDesc }
}

function scoreShipping(change20d: number | null): { score: number; desc: string } {
  if (change20d === null) return { score: 50, desc: 'No shipping data' }
  if (change20d <= SHIP.COLLAPSE) return { score: 85, desc: 'Shipping rates collapsed' }
  if (change20d <= SHIP.WEAK) return { score: 70, desc: 'Shipping rates weak' }
  if (change20d <= SHIP.STABLE) return { score: 45, desc: 'Shipping rates stable' }
  if (change20d <= SHIP.STRONG) return { score: 30, desc: 'Shipping rates firm' }
  return { score: 25, desc: 'Shipping rates surging' }
}

function scoreChinaNews(count: number, total: number): { score: number; desc: string } {
  if (total === 0) return { score: 50, desc: 'No news data' }
  const concentration = count / total
  if (count >= 50) return { score: Math.min(100, 80 + (count - 30) / 2), desc: `Heavy China focus (${count} articles)` }
  if (concentration > 0.30) return { score: 80, desc: `Heavy China focus (${count} articles)` }
  if (concentration > 0.20) return { score: 65, desc: `Elevated China coverage (${count} articles)` }
  if (concentration > 0.10) return { score: 50, desc: `Normal China coverage (${count} articles)` }
...
function calculateChinaTension(
  ...
  totalNews: number,
  specialistSignal: number | null
): { score: number; level: string; regime: string; headline: string; components: ChinaComponents } {
  ...
  const score = Math.max(0, Math.min(100,
    (shipScore * 0.30) + (cnyScore * 0.25) + (fxiScore * 0.20) +
  ...
  return {
    ...
    components: {
      ...
      cny_change_20d: cnyChange20d ? Math.round(cnyChange20d * 1000) / 10 : null,
...
function calculateTariffThreat(
  ...
  specialistSignal: number | null
): { score: number; level: string; regime: string; headline: string; components: TariffComponents } {
  // Component 1: TPU (35%)
  ...
  const score = Math.max(0, Math.min(100,
    (tpuScore * 0.35) + (emvScore * 0.20) +
    (50 + legisAdj) * 0.10 + (50 + specialistAdj) * 0.15 +
    (50 + newsAdj) * 0.20
  ))
...
export async function GET() {
  try {
    ...
    const [
      vixRows, vix3mRows, ovxRows,
      realizedVolRows, vixZlCorrRows, hedgeNewsRows,
      crushRows, oilShare5dRows,
